...
class Espajcpp_test extends CI_Controller {
	...
	public function cpp(){
		...
		/*
		if(file_exists(realpath(".").'/xtemp_tes/'.$rows['spaj_code'].'/'.$rows['spaj_code'].'.pdf')){
			redirect($this->config->item("api_outgoing_url")."/xtemp_tes/".$rows['spaj_code'].'/'.$rows['spaj_code'].'.pdf?'.time());
			exit;
		}
		*/
		...
		$files=scandir(realpath(".")."/xtemp_tes/".$rows['spaj_code']);
		
		$pdf = new \PDFMerger;
		$arr=array();
		foreach($files as $f){
			if(!in_array($f,array('.','..'))){
				$arr[]=realpath(".").'/xtemp_tes/'.$rows['spaj_code'].'/'.$f;
				$pdf->addPDF(realpath(".").'/xtemp_tes/'.$rows['spaj_code'].'/'.$f);
			}
		}
		$pdf->merge('file', realpath(".").'/xtemp_tes/'.$rows['spaj_code'].'/'.$rows['spaj_code'].'.pdf');
	...
	public function cpp_spaj(){
		...
		/*
		if(file_exists(realpath(".").'/xtemp_tes/'.$rows['spaj_code'].'/'.$rows['spaj_code'].'.pdf')){
			redirect($this->config->item("api_outgoing_url")."/xtemp_tes/".$rows['spaj_code'].'/'.$rows['spaj_code'].'.pdf');
			exit;
		}
		*/
	...
	private function cpp1()
	{
		...
		if(!is_dir(realpath(".")."/xtemp_tes/".$rows['spaj_code'])){
			@mkdir(realpath(".")."/xtemp_tes/".$rows['spaj_code']);
		}
		file_put_contents(realpath(".")."/xtemp_tes/".$rows['spaj_code']."/".$rows['spaj_code']."_1.pdf", $output);
	...
	private function cpp2()
	{
		...
		require_once(realpath(".").'/application/libraries/dompdf/autoload.inc.php');
		ob_start();
		$content=file_get_contents(realpath(".")."/template/cpp_page2_tes2.html");
		ob_end_clean();
		...
		foreach($decode3 as $bn){
			...
			foreach ($bn as $key=>$val){
				...
				if($key=="pct"){
					$content=str_replace("{benefit".$jj."}",$val,$content);
				}
				
				if($key=="jenis_kelamin"){
					$content=str_replace("{gen".$jj."}",$val,$content);
				}
		...
		file_put_contents(realpath(".")."/xtemp_tes/".$rows['spaj_code']."/".$rows['spaj_code']."_2.pdf", $output);
	...
	private function cpl2()
	{
		...
		require_once(realpath(".").'/application/libraries/dompdf/autoload.inc.php');
		ob_start();
		$content=file_get_contents(realpath(".")."/template/cpl_page2_tes.html");
		ob_end_clean();
		foreach($decode1 as $key=>$val){
			if($val['name']=="full_name1"){
			$full_name1=$val['value'];
	...
	private function cpl3()
	{
		...
		require_once(realpath(".").'/application/libraries/dompdf/autoload.inc.php');
		ob_start();
		$content=file_get_contents(realpath(".")."/template/cpl_page3_tes.html");
		ob_end_clean();
		/*
		foreach($decode2 as $key=>$val){
			$content=str_replace("{".$key."}",$val,$content);
		...
		*/
		...
		file_put_contents(realpath(".")."/xtemp_tes/".$rows['spaj_code']."/".$rows['spaj_code']."_3.pdf", $output);
