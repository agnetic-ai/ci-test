...
class Syncspajdate extends CI_Controller {
	...
	public function index()
	{
		...
		DATE_FORMAT(m.received_dt,'%Y%m%d') as spaj_date
		from tbl_spaj_pending p
		join tbl_spaj_pending_mapping m on p.span_no=m.span_no and m.pending_type_code='109'
		where ifnull(m.received_dt,'')<>'' and p.source_id='100' 
		and ifnull(p.close_dt,'')=''";
			//echo $sql;
			...
			try{
				...
				foreach($results as $rs){
					...
					$stmts = $dbDB->prepare("exec dbo.usp_sync_spaj_date_original_from_pr '".$rs['spaj_no']."','".$rs['spaj_date']."'");
					$stmts->execute();
					$stmts =null;
